...
export function buildImagePreparationCommand(imagePath, outPath, platform = currentPlatform()) {
...
export function buildImageSizeCommand(imagePath, platform = currentPlatform()) {
    const input = quotePath(imagePath, platform);
    switch (platform) {
        case 'darwin':
...
function imageCommandFailure(action, imagePath, platform, result) {
    ...
    throw new Error(`${action} failed for ${imagePath} (exit ${result.exitCode}): ${detail}`
 ...
 */
export async function prepareImage(ctx, imagePath, cwd, signal, sandboxPolicy) {
    ...
    if (result.exitCode !== 0)
        imageCommandFailure('image preparation', imagePath, platform, result);
...
export async function imageSizeOf(ctx, path, signal, sandboxPolicy) {
    const platform = currentPlatform();
    const result = await ctx.shell.run(ctx.shell.resolve({
    ...
    if (result.exitCode !== 0)
        imageCommandFailure('image size read', path, platform, result);
