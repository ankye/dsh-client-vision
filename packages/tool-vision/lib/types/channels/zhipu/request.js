...
export const ZHIPU_MODELS = ['glm-4v-plus', 'glm-4v-flash'];
 ...
 */
export function requireZhipuEndpoint(baseUrl) {
    const endpoint = (baseUrl ?? '').trim().replace(/\/+$/, '');
    if (endpoint === '') {
        throw new Error(`zhipu has no endpoint (vision.baseUrl); set it in Settings → Plugins → Vision — e.g. ${ZHIPU_DEFAULT_BASE_URL}`);
    }
    return endpoint;
}
