...
export function deviceCaptureHint(mode: 'android' | 'ios'): string {
  return mode === 'android'
 ...
 */
function androidCommand(platform: CapturePlatform, outPath: string, device?: string): string {
...
function quoteDeviceSerial(device: string): string {
  return `'${device}'`
}

/** iOS capture: booted simulator screenshot (macOS host only). */
function iosCommand(platform: CapturePlatform, outPath: string): string {
  if (platform !== 'darwin') {
    throw new Error(
      `take_screenshot mode=ios requires macOS with Xcode (xcrun simctl); current platform is ${platform}`,
    )
  }
